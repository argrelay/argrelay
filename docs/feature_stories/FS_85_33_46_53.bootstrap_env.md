...
The idea is to have a single `@/exe/bootstrap_env.py` script which:
...
Script `@/exe/bootstrap_env.py` is supposed to be version controlled in the target project.
...
There are three scenarios for `@/exe/bootstrap_env.py` (but it can be re-run at any time):
...
Essentially, DEP is what normally called [`constraints.txt`][requirements_txt] and [generated via `pip freeze`][DEP_generation].
...
[DEP_generation]: https://github.com/argrelay/argrelay/blob/v0.8.0.final/exe/bootstrap_env.py#L873
