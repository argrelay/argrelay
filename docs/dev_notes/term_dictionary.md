...
TODO: TODO_66_09_41_16: clarify command line processing:
      `type` and `key` seem obsolete concepts - to be reviewed.
*   type: unique (across all interpreters) name for a set of values.
*   key: unique (within current interpreter) alias name for a type.

*   interrogate (user to specify `arg_value`)
*   suggest (`arg_value`-s to user (to interrogate the user))
...
This is "interrogation" in the sense that suggested `arg_value`-s via Tab-completion appear to user
