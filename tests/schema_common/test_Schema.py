    ...
    def test_type_desc_example_is_loadable_and_dumpable(self):
        ...
        for test_case in test_cases:
            with self.subTest(test_case):
                ...
                loaded_obj = type_desc.object_schema.load(type_desc.dict_example)
                dumped_dict = type_desc.object_schema.dump(loaded_obj)
                # Compare via JSON strings:
                orig_json = type_desc.object_schema.dumps(type_desc.dict_example, sort_keys = True)
                dumped_json = type_desc.object_schema.dumps(dumped_dict, sort_keys = True)
                self.maxDiff = None
                self.assertEqual(
                    orig_json,
                    dumped_json,
