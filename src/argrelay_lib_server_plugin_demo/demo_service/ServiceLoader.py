...
from argrelay_test_infra.test_infra import test_data_
...
class ServiceLoader(AbstractLoader):
    ...
    def load_envelope_collections(
        ...
        self.generate_envelope_id(
            cluster_envelopes + host_envelopes + service_envelopes
        )
    ...
    def generate_envelope_id(
        ...
        for data_envelope in data_envelopes:
            if envelope_id_ not in data_envelope:
                if (
                    data_envelope[ReservedPropName.envelope_class.name]
                    == ServiceEnvelopeClass.class_host.name
                ):
    ...
    def generate_help_hints(
        ...
        host_envelopes = envelope_collections[
            ServiceEnvelopeClass.class_host.name
        ].data_envelopes
        ...
        for host_envelope in host_envelopes:
            ...
            if (
                host_envelope[ReservedPropName.envelope_class.name]
                == ServiceEnvelopeClass.class_host.name
            ):
    ...
    def populate_TD_38_03_48_51_large_generated(
        ...
        for code_maturity in [
            "cm" + str(cmn) for cmn in range(0, self.object_multiplier)
        ]:
            for geo_region in [
                "gr" + str(grn) for grn in range(0, self.object_multiplier)
            ]:
                for flow_stage in [
                    "fs" + str(fsn) for fsn in range(0, self.object_multiplier)
                ]:
                    ...
                    cluster_envelopes = self.switch_loading_class_cluster(
                        cluster_envelopes
                    )
                    ...
                    for host_name in [
                        "hs" + str(hsn) for hsn in range(0, self.object_multiplier)
                    ]:
                        ...
                        for service_name in [
                            "sn{:02d}".format(snn)
                            for snn in range(0, self.object_multiplier)
                        ]:
                            ############################################################################################################
                            # services
