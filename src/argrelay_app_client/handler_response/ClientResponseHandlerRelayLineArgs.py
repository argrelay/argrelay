...
class ClientResponseHandlerRelayLineArgs(ClientResponseHandlerAbstract):
    ...
    def handle_response(
        ...
        invocation_input: InvocationInput = invocation_input_desc.dict_schema.load(
            response_dict
        )
        ElapsedTime.measure("after_object_creation")
        plugin_class: Type[DelegatorAbstract] = import_plugin_class(
            invocation_input.delegator_plugin_entry
        )
