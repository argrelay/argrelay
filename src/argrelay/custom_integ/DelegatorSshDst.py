...
class DelegatorSshDst(DelegatorSingleFuncAbstract):
    ...
    def run_fill_control(
        ...
        if (
            ...
            interp_ctx.curr_container_ipos == interp_ctx.curr_interp.base_container_ipos + ssh_dst_container_ipos_
        ):
            ssh_dst_container = interp_ctx.envelope_containers[(
                # TODO: TODO_73_23_85_93: use helper to select container ipos:
                interp_ctx.curr_interp.base_container_ipos + ssh_dst_container_ipos_
            )]
            any_assignment = (
                set_default_to(ServicePropName.run_mode.name, "active", ssh_dst_container)
                or
                any_assignment
            )
