...
# It is expected that `@/exe/dev_shell.zsh` switches to the target project dir itself (not this script).
...
# Set zsh settings which are (arguably) more convenient.
# These settings force-override those set by user,
# but this is only for `@/exe/dev_shell.zsh` (can be force-overridden again by `@/conf/dev_shell_env.zsh`).
# Official doc:
...
# Source extra config:
if [[ -f "${argrelay_dir}/conf/dev_shell_env.zsh" ]]
then
    source "${argrelay_dir}/conf/dev_shell_env.zsh"
fi

# Enable auto-completion:
source "${argrelay_dir}/exe/shell_env.zsh"

# Restore options:
# In Zsh, restoring options from `setopt` output is more complex than Bash `set +o`.
# For simplicity, we assume standard `set` options here.
