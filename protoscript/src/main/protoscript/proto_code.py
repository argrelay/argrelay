...
class Bootstrapper_state_script_config_file_path(AbstractCachingStateBootstrapper[str]):
    ...
    def _bootstrap_once(
        ...
        return os.path.join(
            ...
            ConfConstPrimer.default_file_basename_conf_primer,
        )
...
class Bootstrapper_state_proj_dir_path_specified(AbstractCachingStateBootstrapper[str]):
    ...
    def _bootstrap_once(
        ...
        return self.env_ctx.bootstrap_state(EnvState.state_parsed_args).proj_dir_path
...
class Bootstrapper_state_script_config_file_data(
    ...
    def _bootstrap_once(
        ...
        if os.path.exists(state_script_config_file_path):
            ...
            file_data = {
                # TODO: This is a rel path from script dir to project dir.
                #       But this can also be a rel path from project dir to script dir (for consistency).
                ConfConstPrimer.field_dir_rel_path_root_proj: os.path.relpath(
                    state_proj_dir_path_specified,
                    state_script_dir_path,
                ),
                ConfConstPrimer.field_file_rel_path_conf_proj: ConfConstProj.default_file_basename_conf_proj,
            }
...
# noinspection PyPep8Naming
class Bootstrapper_state_proj_dir_path_verified(AbstractCachingStateBootstrapper[bool]):
    ...
    def __init__(
        ...
        super().__init__(
            env_ctx=env_ctx,
            env_state=EnvState.state_proj_dir_path_verified,
            state_parents=[
                EnvState.state_proj_dir_path_configured,
    ...
    def _bootstrap_once(
        ...
        if not os.path.isfile(state_proj_conf_man_file_path):
            raise AssertionError(
                f"The `project_dir` `@/` [{state_proj_dir_path_configured}] does not contain the required file [{state_proj_conf_man_file_path}]"
            )
        return True
...
class Bootstrapper_state_proj_conf_man_file_data(
    ...
    def __init__(
        ...
        super().__init__(
            ...
            state_parents=[
                EnvState.state_proj_dir_path_verified,
                EnvState.state_proj_conf_man_file_path,
            ],
        )
    ...
    def _bootstrap_once(
        ...
        state_proj_dir_path_verified = self.env_ctx.bootstrap_state(
            EnvState.state_proj_dir_path_verified
        )
        assert state_proj_dir_path_verified
        ...
        if not os.path.isfile(state_proj_conf_man_file_path):
            raise AssertionError(
                f"file [{state_proj_conf_man_file_path}] is not a file"
            )
...
class Bootstrapper_state_env_conf_man_file_path(AbstractCachingStateBootstrapper[str]):
    ...
    def _bootstrap_once(
        ...
        state_env_conf_man_file_path = os.path.join(
            ...
            ConfConstEnv.default_file_basename_conf_env,
        )
        # TODO: Ensure the path is under with proper error message:
        assert is_sub_path(
            state_env_conf_man_file_path,
            self.env_ctx.bootstrap_state(EnvState.state_env_conf_dir_path),
        )
...
# noinspection PyPep8Naming
class Bootstrapper_state_env_conf_gen_file_created(
    AbstractCachingStateBootstrapper[bool]
):
    ...
    def __init__(
        ...
        super().__init__(
            env_ctx=env_ctx,
            env_state=EnvState.state_env_conf_gen_file_created,
            state_parents=[
                EnvState.state_env_conf_man_file_path,
...
class EnvState(enum.Enum):
    ...
    state_proj_dir_path_verified = Bootstrapper_state_proj_dir_path_verified
    ...
    state_env_conf_gen_file_created = Bootstrapper_state_env_conf_gen_file_created
...
class ConfConstPrimer:
    ...
    # Depends on the location of the script:
    file_abs_path_script = None
    dir_abs_path_script = None
...
class EnvContext:
    ...
    def __init__(
        ...
        self.register_bootstrapper(Bootstrapper_state_proj_dir_path_verified(self))
        ...
        self.register_bootstrapper(Bootstrapper_state_env_conf_gen_file_created(self))

        self.populate_dependencies()

        # TODO: Find "Universal Sink":
        self.universal_sink: EnvState = EnvState.state_env_conf_gen_file_created
    ...
    def do_bootstrap_env(
        ...
        # FS_28_84_41_40: flexible bootstrap
        # `init_venv`
        self.bootstrap_state(EnvState.state_proj_dir_path_verified)
        self.bootstrap_state(EnvState.state_env_conf_dir_path)
        self.ensure_conf_dir()
        self.bootstrap_state(EnvState.state_env_conf_man_file_path)
        self.bootstrap_state(EnvState.state_env_conf_man_file_data)
        self.bootstrap_state(EnvState.state_env_path_to_python)
        self.bootstrap_state(EnvState.state_env_path_to_venv)
    ...
    def ensure_conf_dir(
        ...
        state_env_conf_dir_path = self.bootstrap_state(EnvState.state_env_conf_dir_path)
        state_target_dst_dir_path = self.bootstrap_state(
            EnvState.state_target_dst_dir_path
        )
        if os.path.exists(state_env_conf_dir_path):
            ...
            if state_target_dst_dir_path is None:
                ...
                state_target_dst_dir_path_verified = self.bootstrap_state(
                    EnvState.state_target_dst_dir_path_verified
                )
    ...
    def ensure_allowed_conf_symlink_target(
        ...
        elif ".." in Path(state_target_dst_dir_path).parts:
            raise AssertionError(
    ...
    def recurse_with_required_python_interpreter(
        ...
        state_env_path_to_python = self.bootstrap_state(
            EnvState.state_env_path_to_python
        )
        state_env_path_to_venv = self.bootstrap_state(EnvState.state_env_path_to_venv)

        if is_sub_path(state_env_path_to_python, state_env_path_to_venv):
            state_env_conf_man_file_path = self.bootstrap_state(
                EnvState.state_env_conf_man_file_path
            )
            raise AssertionError(
                f"The `path_to_python` [{state_env_path_to_python}] is a sub-path of the `path_to_venv` [{state_env_path_to_venv}]. "
